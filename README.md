...
Pushing to `main` triggers the GitHub Pages workflow. It installs the locked dependencies with Node.js 22, verifies the generated manifests, runs the masterplan, routing, Museum, asset, article, accuracy, and integrity checks above, builds the Vite application, enforces the initial bundle budgets, and deploys `dist` only after those gates pass.
