...
- Article depth, article claim review, and exhibit review are separate milestones. Depth establishes article completeness, claim review records claim-to-source reconciliation, and exhibit review governs the distinct Museum interpretation; none of the three substitutes for another.
- The panel uses a responsive, object-led reading flow rather than a permanent column split. On desktop, the principal image, caption, short object interpretation, and visitor guide form a compact upper-right reference block. The three untitled prose paragraphs begin at the upper left, flow beside that block, and reclaim the full panel width below it. On narrow screens, the order becomes object image and details, main interpretation, visitor guide, then the records, sources, and article action. The collapsed object record and image rights, collapsed interpretation sources, and full-article action always clear the primary flow and use the full available width.
- The responsive presentation shipped at commit `085e725` is the accepted shared layout baseline. Routine content conversion must use that renderer and CSS without reopening its dimensions, spacing, or responsive behavior.
...
- The principal image or object has three distinct layers: a concise caption identifying what the visitor sees, provenance and rights metadata from the canonical asset registry, and an exhibit-specific interpretation explaining what the object can and cannot establish. Its modal preview preserves the full original composition by default: scale the image down within the bounded preview, center narrower portrait works, and keep a restrained maximum height without distortion or cropping. Cropping is a reviewed exception only when the uncropped image demonstrably fails, and any exception must preserve the focal subject at both desktop and narrow widths. This preview rule does not change the separate wall-installation framing.
- A new or substantially converted main interpretation contains **at least 280 words**, preferably **290–320**, usually in three substantial untitled prose paragraphs. Four untitled paragraphs are valid when the subject reads more naturally that way. It explains the subject directly, incorporates the object without allowing it to redefine the subject, and preserves material uncertainties and disputes from the canonical article. The accepted Kantianism, Marxism, Nāgārjuna, Boethius, and Sextus Empiricus pilots retain their reviewed 250–268-word copy and are not retroactively padded.
- Use a compact visitor guide divided into familiar, subject-appropriate sections such as **Key ideas**, **Central questions**, **Major works**, **Development**, **Influence**, or **Continuing debates**. It belongs with the upper-right object reference on desktop and follows the main interpretation on narrow screens. There is no required section, item count, or shared category template. Each named concept, person, work, school, or tradition must be paired with a brief plain-language explanation rather than presented as an unexplained catalog. The guide should complement the prose, remain concise and scannable, and use restrained vertical spacing.
...
- Accessibility, scannability, and visual balance require editorial and visual judgment. Audits enforce only objective invariants such as required fields, word floors, paragraph shape, status prerequisites, current locks, relationships, and provenance completeness; they must not manufacture automated scores for prose quality or layout balance.
...
- `standard-compliant` — the article relationship is unambiguous, the article is currently `claim-reviewed`, factual exhibit content is reconciled with that article and its sources, the exhibit lock is current, and the presentation satisfies the locked objective contract plus completed editorial and visual review.
...
7. Every converted primary exhibit preview is checked at representative desktop and narrow widths with the full composition visible by default, portrait works centered, and faces or other focal subjects recognizable. Any intentional crop is recorded and visually verified at both widths; wall-installation framing is reviewed separately.
