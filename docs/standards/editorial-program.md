...
The durable reviewed-exhibit contract is defined in the [Museum exhibit-wall standard](../museum-masterplan/exhibit-wall-standard.md). A reviewed primary exhibit uses a responsive object-led reading flow: a compact upper-right object and visitor-guide reference on desktop allows the prose to wrap beside it and then reclaim the full width, while narrow screens use the documented single-column order. It also uses a roughly 32–35-word direct wall invitation; provides object caption, provenance, and interpretation; presents 250–268 words in three untitled prose paragraphs; adds a compact, subject-appropriate visitor guide whose named concepts, people, works, and traditions receive plain-language explanation; and links to the full sourced article. The guide has no fixed item count or universal category template. Article review and exhibit review remain separate locked states, and presentation conformance never substitutes for claim review or factual reconciliation.
