...
The durable reviewed-exhibit contract is defined in the [Museum exhibit-wall standard](../museum-masterplan/exhibit-wall-standard.md). Article depth, article claim review, and exhibit review are separate milestones. A reviewed primary exhibit uses the responsive object-led reading flow accepted at commit `085e725`: a compact upper-right object and visitor-guide reference on desktop allows the prose to wrap beside it and then reclaim the full width, while narrow screens use the documented single-column order. It also uses a roughly 32–35-word direct wall invitation; provides object caption, provenance, and interpretation; gives each new or substantially converted exhibit at least 280 main-interpretation words, preferably 290–320, usually in three substantial untitled paragraphs and four when more natural; adds a compact, subject-appropriate visitor guide whose named concepts, people, works, and traditions receive plain-language explanation; and links to the full sourced article. The five accepted pilots retain their reviewed 250–268-word copy without padding. The guide has no fixed item count or universal category template. An exhibit becomes `standard-compliant` only after its corresponding article is currently `claim-reviewed`, its factual content is reconciled with that article and its sources, its exhibit lock is current, and editorial and visual review confirms the locked presentation. Automated audits enforce objective invariants only; accessibility, explanation quality, and visual balance remain editorial judgments.
