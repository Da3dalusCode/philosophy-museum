...
This index separates current production authority from historical design and implementation evidence. The deployed state is determined by `main` and the GitHub Pages workflow. When narrative documentation disagrees with generated data or an executable audit, the generated data and passing audit are authoritative.
...
The current validated Museum contains 26 curated/open galleries, 105 rooms, 189 primary exhibits, 406 supplemental exhibits, and 595 interpreted stops. Counts in archived documents describe their own checkpoints, not the current release.
...
Alternative diagrams and migration records under [`museum-masterplan/`](./museum-masterplan/) explain how the approved design was selected and implemented. The Continuous Enfilade plan and manifest remain current; Ring, campus, and braided-promenade materials are design history.
