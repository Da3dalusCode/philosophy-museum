"""Build and verify the locked Gallery 14–15 media derivatives."""
...
MANIFEST_PATH = ROOT / "scripts" / "museumSuccessorGalleriesAssetManifest.json"
...
VALID_HALL_FOLDERS = {"hellenistic-roman-ways", "late-antiquity-inheritance"}
...
def load_manifest(refresh_locks: bool) -> dict[str, dict[str, object]]:
    ...
    if len(assets) != EXPECTED_ASSET_COUNT:
        raise RuntimeError(f"Expected {EXPECTED_ASSET_COUNT} successor-gallery assets, found {len(assets)}.")
    for slug, record in assets.items():
        folder = record.get("hallFolder")
        if folder not in VALID_HALL_FOLDERS:
            raise RuntimeError(f"{slug}.hallFolder is not an approved successor-gallery folder.")
...
def assert_locked(slug: str, variant: str, actual: dict[str, int | str], expected: object) -> None:
    ...
    differences = [
        field for field in ("width", "height", "bytes", "sha256")
...
def main() -> None:
    ...
    if unknown:
        raise RuntimeError(f"Unknown successor-gallery asset selection: {', '.join(sorted(unknown))}")
        ...
        for index, (slug, record) in enumerate(assets.items(), start=1):
            ...
            if temporary is None:
                temporary = Path(stack.enter_context(
                    tempfile.TemporaryDirectory(prefix="philosophy-atlas-successor-gallery-assets-")
                ))
    ...
    if args.refresh_locks:
        ...
        print(f"Refreshed {processed * 2} successor-gallery derivative locks.")
    else:
        print(f"Verified {processed * 2} locked successor-gallery derivatives.")
