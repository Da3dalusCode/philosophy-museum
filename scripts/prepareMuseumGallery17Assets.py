"""Build and verify the locked Gallery 17 media derivatives."""
...
MANIFEST_PATH = ROOT / "scripts" / "museumGallery17AssetManifest.json"
...
HALL_FOLDER = "empiricism-science-political-order"
...
MAX_TEXT_DOMINANT_OR_SINGLE_BOOK = 3
...
def load_manifest(refresh_locks: bool) -> dict[str, dict[str, object]]:
    ...
    if len(assets) != EXPECTED_ASSET_COUNT:
        raise RuntimeError(f"Expected {EXPECTED_ASSET_COUNT} Gallery 17 assets, found {len(assets)}.")

    flagged_visuals = 0
    for slug, record in assets.items():
        if not isinstance(record, dict):
            raise RuntimeError(f"{slug} must be an object.")
        if record.get("hallFolder") != HALL_FOLDER:
            raise RuntimeError(f"{slug}.hallFolder must be {HALL_FOLDER}.")
        ...
        if visual_character == "text-dominant" or record.get("textDominantOrSingleBook") is True:
            flagged_visuals += 1
        if record.get("textDominantOrSingleBook") is not False:
            raise RuntimeError(
                f"{slug}.textDominantOrSingleBook must explicitly be false for the approved Gallery 17 program."
            )
        for maximum_field in ("sceneMaximum", "panelMaximum"):
            ...
            if not isinstance(maximum, int) or maximum < MIN_DERIVATIVE_EDGE:
                raise RuntimeError(f"{slug}.{maximum_field} must be an integer at least {MIN_DERIVATIVE_EDGE}.")
    ...
    if flagged_visuals > MAX_TEXT_DOMINANT_OR_SINGLE_BOOK:
        raise RuntimeError(
            "Gallery 17 exceeds the approved maximum of "
            f"{MAX_TEXT_DOMINANT_OR_SINGLE_BOOK} text-dominant or lone-book images."
        )
...
def main() -> None:
    ...
    if unknown:
        raise RuntimeError(f"Unknown Gallery 17 asset selection: {', '.join(sorted(unknown))}")
    ...
    with ExitStack() as stack:
        ...
        for index, (slug, record) in enumerate(assets.items(), start=1):
            ...
            if temporary is None:
                temporary = Path(
                    stack.enter_context(
                        tempfile.TemporaryDirectory(prefix="philosophy-atlas-gallery-17-assets-")
                    )
                )
    ...
    if args.refresh_locks:
        ...
        print(f"Refreshed {processed * 2} Gallery 17 derivative locks.")
    else:
        print(f"Verified {processed * 2} locked Gallery 17 derivatives.")
