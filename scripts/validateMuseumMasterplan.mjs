...
check(canonicalRooms.length === 90, `Canonical live program must contain 90 rooms, found ${canonicalRooms.length}`);
check(canonicalExhibits.length === 171, `Canonical live program must contain 171 primary exhibits, found ${canonicalExhibits.length}`);
...
check(plannedLiveAssignments.length === 171, `Masterplan assigns ${plannedLiveAssignments.length}, not 171, primaries to the twenty-two live halls`);
...
check(MUSEUM_LIVE_PROGRAM_TOTALS.hallCount === 22 && MUSEUM_LIVE_PROGRAM_TOTALS.roomCount === 90 && MUSEUM_LIVE_PROGRAM_TOTALS.exhibitCount === 171, 'Exported live program totals are stale');
check(MUSEUM_LIVE_PROGRAM_TOTALS.recordCapacity === 236 && MUSEUM_LIVE_PROGRAM_TOTALS.reserveCapacity === 65, 'Live program capacity totals must be 236 capacity / 65 reserve');
check(same(MUSEUM_LIVE_PROGRAM_TOTALS.tierCounts, EXPECTED_LIVE_TIERS), 'Exported live tier totals are stale');
check(MUSEUM_LIVE_HALL_TOTALS.length === 22, 'Exported live hall totals must contain twenty-two records');
check(MUSEUM_LIVE_ROOM_TOTALS.length === 90, 'Exported live room totals must contain 90 records');
...
check(MUSEUM_LIVE_LEGACY_EXHIBIT_COMPATIBILITY.length === 43, `Expected 43 carried legacy exhibits, found ${MUSEUM_LIVE_LEGACY_EXHIBIT_COMPATIBILITY.length}`);
check(MUSEUM_LEGACY_EXHIBIT_COMPATIBILITY.length === 5, `Expected 5 displaced legacy exhibits, found ${MUSEUM_LEGACY_EXHIBIT_COMPATIBILITY.length}`);
...
check(activeBuildingManifest.counts?.curatedOpen === 22, 'Active manifest must contain 22 curated/open galleries');
check(activeBuildingManifest.counts?.plannedWalkable === 4, 'Active manifest must contain 4 planned/walkable galleries');
...
check(activeCuratedNodes.length === 22, `Active manifest exposes ${activeCuratedNodes.length}, not 22, curated galleries`);
check(activePlannedNodes.length === 4, `Active manifest exposes ${activePlannedNodes.length}, not 4, planned shells`);
check(same(sorted(activeCuratedNodes.map(({publicHallId}) => publicHallId)), sorted(APPROVED_HALL_IDS)), 'Active curated roster differs from the canonical twenty-two');
...
console.log('  canonical live subset: 22 halls · 90 rooms · 171 primary exhibits · 236 capacity · 65 reserve');
console.log('  tiers: 82 anchor · 75 standard · 8 supporting · 5 cluster · 1 archive');
console.log('  compatibility: 43 carried legacy routes · 5 truthful not-installed handoffs');
console.log('  active building: Continuous Enfilade · 26 galleries · 105 rooms · 22 curated/open · 4 planned/walkable');
