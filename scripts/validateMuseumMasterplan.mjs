...
check(canonicalRooms.length === 98, `Canonical live program must contain 98 rooms, found ${canonicalRooms.length}`);
check(canonicalExhibits.length === 183, `Canonical live program must contain 183 primary exhibits, found ${canonicalExhibits.length}`);
...
check(plannedLiveAssignments.length === 183, `Masterplan assigns ${plannedLiveAssignments.length}, not 183, primaries to the twenty-four live halls`);
...
check(MUSEUM_LIVE_PROGRAM_TOTALS.hallCount === 24 && MUSEUM_LIVE_PROGRAM_TOTALS.roomCount === 98 && MUSEUM_LIVE_PROGRAM_TOTALS.exhibitCount === 183, 'Exported live program totals are stale');
check(MUSEUM_LIVE_PROGRAM_TOTALS.recordCapacity === 250 && MUSEUM_LIVE_PROGRAM_TOTALS.reserveCapacity === 67, 'Live program capacity totals must be 250 capacity / 67 reserve');
check(same(MUSEUM_LIVE_PROGRAM_TOTALS.tierCounts, EXPECTED_LIVE_TIERS), 'Exported live tier totals are stale');
check(MUSEUM_LIVE_HALL_TOTALS.length === 24, 'Exported live hall totals must contain twenty-four records');
check(MUSEUM_LIVE_ROOM_TOTALS.length === 98, 'Exported live room totals must contain 98 records');
...
check(MUSEUM_LIVE_LEGACY_EXHIBIT_COMPATIBILITY.length === 46, `Expected 46 carried legacy exhibits, found ${MUSEUM_LIVE_LEGACY_EXHIBIT_COMPATIBILITY.length}`);
check(MUSEUM_LEGACY_EXHIBIT_COMPATIBILITY.length === 2, `Expected 2 displaced legacy exhibits, found ${MUSEUM_LEGACY_EXHIBIT_COMPATIBILITY.length}`);
...
check(activeBuildingManifest.counts?.curatedOpen === 24, 'Active manifest must contain 24 curated/open galleries');
check(activeBuildingManifest.counts?.plannedWalkable === 2, 'Active manifest must contain 2 planned/walkable galleries');
...
check(activeCuratedNodes.length === 24, `Active manifest exposes ${activeCuratedNodes.length}, not 24, curated galleries`);
check(activePlannedNodes.length === 2, `Active manifest exposes ${activePlannedNodes.length}, not 2, planned shells`);
check(same(sorted(activeCuratedNodes.map(({publicHallId}) => publicHallId)), sorted(APPROVED_HALL_IDS)), 'Active curated roster differs from the canonical twenty-four');
...
console.log('  canonical live subset: 24 halls · 98 rooms · 183 primary exhibits · 250 capacity · 67 reserve');
console.log('  tiers: 89 anchor · 79 standard · 9 supporting · 5 cluster · 1 archive');
console.log('  compatibility: 46 carried legacy routes · 2 truthful not-installed handoffs');
console.log('  active building: Continuous Enfilade · 26 galleries · 105 rooms · 24 curated/open · 2 planned/walkable');
