...
const HALL_ID = 'hellenistic-roman-ways';
const EXPECTED_COUNT = 7;
const LEDGER_JSON_URL = new URL('../docs/editorial/gallery-02-supplemental-review-ledger.json', import.meta.url);
...
const verifyVariant = async (asset, variantKind, issues) => {
  const variant = asset?.variants?.[variantKind];
  if (!variant?.path) {
    issues.push(`${variantKind} asset path is missing`);
    return;
  }
  const manifestVariant = assetManifest[asset.id]?.[variantKind];
  if (!manifestVariant) {
    issues.push(`${variantKind} asset is absent from the acquisition manifest`);
    return;
  }
  try {
    const bytes = await readFile(new URL(variant.path, PUBLIC_URL));
    const sha256 = createHash('sha256').update(bytes).digest('hex');
    if (bytes.length !== manifestVariant.bytes) issues.push(`${variantKind} byte count differs from the acquisition manifest`);
    if (sha256 !== manifestVariant.sha256) issues.push(`${variantKind} checksum differs from the acquisition manifest`);
    if (variant.width !== manifestVariant.width || variant.height !== manifestVariant.height) {
      issues.push(`${variantKind} runtime dimensions differ from the installed asset`);
    }
  } catch {
    issues.push(`${variantKind} asset file is missing`);
  }
};

const entries = await Promise.all(registryEntries.map(async ({exhibit, layout}) => {
  ...
  if (/Visitor Guide|How to read this exhibit|previously displayed|audit|change-?log|\b(?:modal|panel|room) object\b/iu.test(visitorCopy)) {
    issues.push('visitor-facing copy retains instructional, internal, audit, or change-log language');
  }
  ...
  if (layout.assetId !== exhibit.assetId || exhibit.panelAssetId !== exhibit.assetId) issues.push('panel or physical asset differs from the reviewed record asset');
...
const markdown = `# Gallery 02 supplemental review ledger
...
Regenerate with \`npm run report:gallery02-supplementals\` and verify with \`npm run audit:gallery02-supplementals\`.
...
if (write) {
  ...
  console.log(`Wrote Gallery 02 supplemental ledger for ${entries.length} registry records.`);
} else {
  let existing;
  try {
    existing = await readFile(LEDGER_JSON_URL, 'utf8');
  } catch {
    errors.push('generated ledger is missing; run npm run report:gallery02-supplementals');
  }
  if (existing !== undefined && existing !== serialized) errors.push('generated ledger is stale; run npm run report:gallery02-supplementals');
}
...
if (errors.length) {
  ...
  console.log(`Gallery 02 supplemental audit passed: ${entries.length} registry records, ${ledger.resolvedCount} resolved.`);
}
