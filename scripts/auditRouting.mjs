...
check('Museum convenience, hall, and exhibit routes parse and serialize', () => {
  ...
  assert.equal(exhibitCount, 192);
  assert.equal(MUSEUM_SUPPLEMENTAL_EXHIBITS.length, 411);
  assert.equal(exhibitCount + MUSEUM_SUPPLEMENTAL_EXHIBITS.length, 603, 'The Museum directory must expose 603 interpreted stops');
...
check('serializers emit the required literal route families', () => {
  ...
  assert.equal(
    serializeHashRoute({kind: 'museum', hallId: 'critique-power-deconstruction', exhibitId: 'foucault'}),
    '#/museum/critique-power-deconstruction/exhibits/foucault',
  );
  assert.equal(
