"""Build and verify the locked Gallery 20 and 21 media derivatives."""
...
MANIFEST_PATH = ROOT / "scripts" / "museumGalleries20And21AssetManifest.json"
...
def load_manifest(refresh_locks: bool) -> dict[str, dict[str, object]]:
    ...
    if len(assets) != EXPECTED_ASSET_COUNT:
        raise RuntimeError(
            f"Expected {EXPECTED_ASSET_COUNT} Gallery 20/21 assets, found {len(assets)}."
        )
    ...
    for slug, record in assets.items():
        ...
        if visual_character == "text-dominant" or record.get("textDominantOrSingleBook") is not False:
            raise RuntimeError(
                f"{slug} violates the zero paper-only/lone-book gate for Galleries 20 and 21."
            )
    ...
    if folder_counts != EXPECTED_FOLDER_COUNTS:
        raise RuntimeError(
            f"Gallery 20/21 folder counts drifted: expected {EXPECTED_FOLDER_COUNTS}, got {folder_counts}."
        )
...
def main() -> None:
    ...
    if unknown:
        raise RuntimeError(f"Unknown Gallery 20/21 asset selection: {', '.join(sorted(unknown))}")
                ...
                temporary = Path(
                    stack.enter_context(
                        tempfile.TemporaryDirectory(
                            prefix="philosophy-atlas-galleries-20-21-assets-"
                        )
                    )
                )
    ...
    if args.refresh_locks:
        ...
        print(f"Refreshed {processed * 2} Gallery 20/21 derivative locks.")
    else:
        print(f"Verified {processed * 2} locked Gallery 20/21 derivatives.")
