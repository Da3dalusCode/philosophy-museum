"""Build and verify the locked Gallery 13 and Gallery 16 media derivatives."""
...
MANIFEST_PATH = ROOT / "scripts" / "museumGalleries13And16AssetManifest.json"
...
VALID_HALL_FOLDERS = {"latin-christian-scholastic", "rationalism-mind-nature-system"}
...
def load_manifest(refresh_locks: bool) -> dict[str, dict[str, object]]:
    ...
    if len(assets) != EXPECTED_ASSET_COUNT:
        raise RuntimeError(f"Expected {EXPECTED_ASSET_COUNT} Gallery 13/16 assets, found {len(assets)}.")
    for slug, record in assets.items():
        folder = record.get("hallFolder")
        if folder not in VALID_HALL_FOLDERS:
            raise RuntimeError(f"{slug}.hallFolder is not an approved Gallery 13/16 folder.")
...
def main() -> None:
    ...
    if unknown:
        raise RuntimeError(f"Unknown Gallery 13/16 asset selection: {', '.join(sorted(unknown))}")
        ...
        for index, (slug, record) in enumerate(assets.items(), start=1):
            ...
            if temporary is None:
                temporary = Path(stack.enter_context(
                    tempfile.TemporaryDirectory(prefix="philosophy-atlas-gallery-13-16-assets-")
                ))
    ...
    if args.refresh_locks:
        ...
        print(f"Refreshed {processed * 2} Gallery 13/16 derivative locks.")
    else:
        print(f"Verified {processed * 2} locked Gallery 13/16 derivatives.")
