...
export function EditorialMethodology() {
  ...
  return <div className="page methodology-page">
    ...
    <section className="methodology-section">
      <h2>Phased review</h2>
      <p>The atlas contains a large local corpus. The universal depth migration and the current canonical claim-review pass are complete, but neither is a claim of permanent correctness or external academic validation. The current supplemental state is {supplementalExhibits.reviewed}/{supplementalExhibits.total} reviewed with {supplementalExhibits.backlog === 0 ? 'zero backlog' : `${supplementalExhibits.backlog} in backlog`}. Review continues through stale-lock maintenance, corrections, and evidence checks for future changes, with high-risk biography, quotation, chronology, influence, origin, and cross-cultural claims prioritized. The public status on each page—not the presence of polished or lengthy prose—indicates the recorded review state.</p>
    </section>
  </div>;
}
