...
export function PhilosophyWall({initialMode='overview',genealogy=false,href}:{initialMode?:WallMode;genealogy?:boolean;href:RouteHref}){
  ...
  const onPointerDown=(event:React.PointerEvent<HTMLDivElement>)=>{if(!engaged)return;pointerRef.current={id:event.pointerId,x:event.clientX,y:event.clientY,left:event.currentTarget.scrollLeft,dragged:false};suppressClickRef.current=false;event.currentTarget.setPointerCapture(event.pointerId)};
  const onPointerMove=(event:React.PointerEvent<HTMLDivElement>)=>{const pointer=pointerRef.current;if(!engaged||!pointer||pointer.id!==event.pointerId)return;const dx=event.clientX-pointer.x;const dy=event.clientY-pointer.y;if(Math.hypot(dx,dy)>7)pointer.dragged=true;if(Math.abs(dx)>Math.abs(dy)){event.preventDefault();event.currentTarget.scrollLeft=pointer.left-dx}};
