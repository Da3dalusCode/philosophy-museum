...
import {ArrowRight,BookOpen,ChevronLeft,ChevronRight,Filter,GitBranch,HelpCircle,Landmark,Minus,Network,Plus,RotateCcw,Search,UserRound,X,XCircle} from 'lucide-react';
...
export function PhilosophyWall({initialMode='overview',genealogy=false,onBranch,onPhilosopher}:{initialMode?:WallMode;genealogy?:boolean;onBranch:(id:string)=>void;onPhilosopher:(id:string)=>void}){
  const[mode,setMode]=useState<WallMode>(initialMode);const[density,setDensity]=useState<Density>('compact');const[zoom,setZoom]=useState<number>(1);const[query,setQuery]=useState('');const[period,setPeriod]=useState('all');const[schoolId,setSchoolId]=useState('all');const[itemType,setItemType]=useState<ItemType>('all');const[selected,setSelected]=useState<Selected>(null);
  ...
  const setChartZoom=(next:number)=>{const clamped=Math.min(ZOOM_MAX,Math.max(ZOOM_MIN,Math.round(next*20)/20));const scroller=scrollRef.current;const center=scroller?(scroller.scrollLeft+scroller.clientWidth/2)/zoom:0;setZoom(clamped);requestAnimationFrame(()=>{if(scroller)scroller.scrollLeft=Math.max(0,center*clamped-scroller.clientWidth/2)})};
  ...
  return <section className={`philosophy-wall mode-${mode} density-${density} ${selected?'has-selection':''}`}>
    ...
    <div className="wall-controls">
      <div className="wall-modes" aria-label="Wall emphasis mode">{modeOptions.map(item=><button key={item.id} className={mode===item.id?'active':''} onClick={()=>setMode(item.id)}><b>{item.label}</b><small>{item.description}</small></button>)}</div>
      <div className="density-control" aria-label="Timeline density"><span>Density</span><button className={density==='comfortable'?'active':''} onClick={()=>setDensity('comfortable')}>Comfortable</button><button className={density==='compact'?'active':''} onClick={()=>setDensity('compact')}>Compact</button></div>
    </div>
    ...
    {empty?<div className="empty-state static-info-card"><XCircle/><h2>No wall-chart items match</h2><p>Broaden the search or reset the period, school, and type filters.</p><button className="btn btn-secondary" onClick={reset}><RotateCcw size={15}/> Reset filters</button></div>:<>
      ...
      <div className="wall-scroll" ref={scrollRef} tabIndex={0} aria-label="Scrollable Philosophy Wall chart"><div className="wall-zoom-stage" style={{width:WALL_WIDTH*zoom,height:canvasHeight*zoom}}><div className="wall-canvas" style={{width:WALL_WIDTH,height:canvasHeight,transform:`scale(${zoom})`}}>
    ...
    {selected&&<WallDrawer selected={selected} onClose={()=>setSelected(null)} onBranch={onBranch} onPhilosopher={onPhilosopher} onSelect={setSelected}/>}
  </section>
}
...
function WallDrawer({selected,onClose,onBranch,onPhilosopher,onSelect}:{selected:NonNullable<Selected>;onClose:()=>void;onBranch:(id:string)=>void;onPhilosopher:(id:string)=>void;onSelect:(selected:Selected)=>void}){
  ...
  return <><button className="drawer-backdrop" aria-label="Close wall-chart details" onClick={onClose}/><aside className="detail-drawer wall-drawer" style={{'--accent':accent} as React.CSSProperties}><button className="drawer-close btn btn-ghost" onClick={onClose}><X size={18}/> Close</button>
    {school&&<SchoolDrawer school={school} onBranch={onBranch} onPhilosopher={onPhilosopher} onSelect={onSelect}/>}
    {philosopher&&<PhilosopherDrawer philosopher={philosopher} onBranch={onBranch} onPhilosopher={onPhilosopher} onSelect={onSelect}/>}
    {work&&<WorkDrawer work={work} onBranch={onBranch} onPhilosopher={onPhilosopher}/>}
    {event&&<EventDrawer event={event} onBranch={onBranch} onPhilosopher={onPhilosopher}/>}
  </aside></>;
}

function SchoolDrawer({school,onBranch,onPhilosopher,onSelect}:{school:WallSchool;onBranch:(id:string)=>void;onPhilosopher:(id:string)=>void;onSelect:(selected:Selected)=>void}){
  ...
  return <><div className="placard-number">School active period · {periodFor(school.startYear)?.name}</div><div className="eyebrow">{formatWallYear(school.startYear)}–{formatWallYear(school.endYear)}</div><h2>{school.name}</h2><p className="placard-lead">{school.summary}</p>{school.branchId&&<button className="btn btn-primary drawer-primary" onClick={()=>onBranch(school.branchId!)}>Open in Branch Explorer <ArrowRight size={15}/></button>}<section><h3><UserRound size={15}/> Thinkers on this band</h3>{school.philosopherIds.map(id=>{const person=philosopherById(id);return person?<button className="btn btn-secondary connection-button" key={id} onClick={()=>onPhilosopher(id)}><PhilosopherPortrait philosopher={person}/><span>{person.name}<small>{person.lifespan}</small></span><ArrowRight size={13}/></button>:null})}</section><section><h3><Network size={15}/> Direct routes</h3>{routes.map(route=>{const id=route.sourceId===school.id?route.targetId:route.sourceId;return <button className="btn btn-secondary connection-button" key={route.id} onClick={()=>onSelect({type:'school',id})}><span className="branch-dot" style={{background:relationColors[route.kind]}}/><span>{wallSchools.find(item=>item.id===id)?.name}<small>{relationLabels[route.kind]}</small></span><ArrowRight size={13}/></button>})}</section>{works.length>0&&<section><h3><BookOpen size={15}/> Landmark works</h3>{works.map(item=><button className="btn btn-secondary connection-button" key={item.id} onClick={()=>onSelect({type:'work',id:item.id})}><span>{item.title}<small>{formatWallYear(item.year)}</small></span><ArrowRight size={13}/></button>)}</section>}</>;
}

function PhilosopherDrawer({philosopher,onBranch,onPhilosopher,onSelect}:{philosopher:NonNullable<ReturnType<typeof philosopherById>>;onBranch:(id:string)=>void;onPhilosopher:(id:string)=>void;onSelect:(selected:Selected)=>void}){
  ...
  return <><div className="placard-number">Philosopher on the wall</div><div className="eyebrow">{philosopher.lifespan} · {philosopher.tradition}</div><h2>{philosopher.name}</h2><p className="placard-lead">{philosopher.contributionSummary}</p><section className="why-matters static-info-card"><span>Main contribution</span><b>{philosopher.mainIdeas.join(' · ')}</b><p>{philosopher.beginnerExplanation}</p></section><button className="btn btn-primary drawer-primary" onClick={()=>onPhilosopher(philosopher.id)}>Open full profile <ArrowRight size={15}/></button><section><h3><GitBranch size={15}/> Schools & branches</h3>{schools.map(school=><button className="btn btn-secondary connection-button" key={school.id} onClick={()=>onSelect({type:'school',id:school.id})}><span className="branch-dot" style={{background:school.color}}/><span>{school.name}</span><ArrowRight size={13}/></button>)}{philosopher.primaryBranchIds.map(id=>branchById(id)?<button className="btn btn-secondary connection-button" key={id} onClick={()=>onBranch(id)}><span className="branch-dot" style={{background:branchById(id)?.color}}/><span>{branchById(id)?.name}</span><ArrowRight size={13}/></button>:null)}</section><section><h3><BookOpen size={15}/> Key works</h3><div className="drawer-static-list">{philosopher.keyWorks.map(title=><span className="tag-static" key={title}>{title}</span>)}</div>{works.map(item=><button className="btn btn-secondary connection-button" key={item.id} onClick={()=>onSelect({type:'work',id:item.id})}><span>{item.title}<small>{formatWallYear(item.year)}</small></span><ArrowRight size={13}/></button>)}</section><section><h3><Network size={15}/> Influence neighborhood</h3>{neighboring.map(route=><div className="drawer-moment static-info-card" key={route.id}><small>{relationLabels[route.kind]}</small><b>{route.summary}</b></div>)}</section></>;
}

function WorkDrawer({work,onBranch,onPhilosopher}:{work:WallWork;onBranch:(id:string)=>void;onPhilosopher:(id:string)=>void}){
  const author=philosopherById(work.authorId??'');return <><div className="placard-number">Landmark work · {periodFor(work.year)?.name}</div><div className="eyebrow">{formatWallYear(work.year)}</div><h2>{work.title}</h2><p className="placard-lead">{work.summary}</p>{author&&<section><h3><UserRound size={15}/> Author</h3><button className="btn btn-secondary connection-button" onClick={()=>onPhilosopher(author.id)}><PhilosopherPortrait philosopher={author}/><span>{author.name}</span><ArrowRight size={13}/></button></section>}<section><h3><GitBranch size={15}/> Connected branches</h3>{work.branchIds.map(id=>branchById(id)?<button className="btn btn-secondary connection-button" key={id} onClick={()=>onBranch(id)}><span className="branch-dot" style={{background:branchById(id)?.color}}/><span>{branchById(id)?.name}</span><ArrowRight size={13}/></button>:null)}</section></>;
}

function EventDrawer({event,onBranch,onPhilosopher}:{event:TimelineEvent;onBranch:(id:string)=>void;onPhilosopher:(id:string)=>void}){
  return <><div className="placard-number">Historical turn · {event.era}</div><div className="eyebrow">{formatWallYear(event.year,event.approximate)}</div><h2>{event.title}</h2><p className="placard-lead">{event.description}</p><section className="why-matters static-info-card"><span>Why this matters</span><b>{event.whyItMatters}</b></section>{event.relatedWorkIds&&<section><h3><BookOpen size={15}/> Related works</h3><div className="drawer-static-list">{event.relatedWorkIds.map(title=><span className="tag-static" key={title}>{title}</span>)}</div></section>}<section><h3><GitBranch size={15}/> Branches</h3>{event.branchIds.map(id=>branchById(id)?<button className="btn btn-secondary connection-button" key={id} onClick={()=>onBranch(id)}><span className="branch-dot" style={{background:branchById(id)?.color}}/><span>{branchById(id)?.name}</span><ArrowRight size={13}/></button>:null)}</section><section><h3><UserRound size={15}/> Philosophers</h3>{event.philosopherIds.map(id=>{const person=philosopherById(id);return person?<button className="btn btn-secondary connection-button" key={id} onClick={()=>onPhilosopher(id)}><PhilosopherPortrait philosopher={person}/><span>{person.name}</span><ArrowRight size={13}/></button>:null})}</section></>;
}
