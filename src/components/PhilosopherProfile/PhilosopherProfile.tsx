...
export function PhilosopherProfile({route,href}:{route:PhilosopherRoute;href:RouteHref}){
  ...
  return <div className="page philosopher-atlas-page compact-content-page"><PageHead eyebrow="Thinkers in context" title="Philosopher Profiles" text="Biographies, questions, works, and influence routes show how a thinker changed the conversation."/>
    <div className="profiles-layout"><aside className="profile-list"><label><Search size={15}/><input aria-label="Search philosophers" value={q} onChange={e=>setQ(e.target.value)} placeholder="Find a thinker, tradition, or idea…"/></label><div className="profile-list-scroll">{list.map(x=><a className={`selectable-card ${p.id===x.id?'active is-selected':''}`} key={x.id} href={href({kind:'philosopher',philosopherId:x.id})} aria-current={p.id===x.id?'page':undefined}><PhilosopherPortrait philosopher={x}/><span><b>{x.name}</b><small>{x.dateDisplay??x.lifespan} · {x.tradition}</small></span><ArrowRight size={13}/></a>)}</div></aside>
