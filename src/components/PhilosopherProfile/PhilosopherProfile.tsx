...
import {ArrowRight,BookOpen,Clock3,GitCompareArrows,GitBranch,HelpCircle,Landmark,Network,Search,Scale,UserRound} from 'lucide-react';
...
import {CitedText, EditorialReferences, EditorialStatusPanel} from '../Editorial/EditorialEvidence';
  ...
  return <div className="page philosopher-atlas-page compact-content-page"><PageHead eyebrow="Thinkers in context" title="Philosopher Profiles" text="Biographies, questions, works, and influence routes show how a thinker changed the conversation."/>
    <div className="profiles-layout"><aside className="profile-list"><label><Search size={15}/><input aria-label="Search philosophers" value={q} onChange={e=>setQ(e.target.value)} placeholder="Find a thinker, tradition, or idea…"/></label><div className="profile-list-scroll" ref={listRef}>{list.map(x=><a ref={p.id===x.id?activeRef:undefined} className={`selectable-card ${p.id===x.id?'active is-selected':''}`} key={x.id} href={href({kind:'philosopher',philosopherId:x.id})} aria-current={p.id===x.id?'page':undefined}><PhilosopherPortrait philosopher={x}/><span><b>{x.name}</b><small>{x.dateDisplay??x.lifespan} · {x.tradition}</small></span><ArrowRight size={13}/></a>)}</div></aside>
      <article className={`profile-detail deep-profile ${p.articleSections?'article-page':''}`} style={{'--accent':p.color} as React.CSSProperties}>
        <div className="profile-hero"><PhilosopherPortrait philosopher={p} size="large"/><div><div className="eyebrow"><CitedText record={p} claimKey="classification">{p.region} · {p.tradition}</CitedText></div><h1>{p.name}</h1><p><CitedText record={p} claimKey="date">{displayDate}</CitedText></p>{p.dateConfidence&&<small className="image-credit">Date confidence: {p.dateConfidence}</small>}{image?.url&&<small className="image-credit">{image.credit}{image.license?` · ${image.license}`:''}</small>}</div></div>
        <EditorialStatusPanel record={p} href={href}/>
        ...
        <section className="lead-card static-info-card"><Landmark/><div><span>Short biography</span><h2><CitedText record={p} claimKey="contribution-summary">{p.contributionSummary}</CitedText></h2><p><CitedText record={p} claimKey="short-biography">{p.shortBio??p.beginnerExplanation}</CitedText></p></div></section>
        <div className="profile-grid profile-context-grid"><section className="static-info-card"><h3><Clock3 size={14}/> Historical context</h3><p><CitedText record={p} claimKey="historical-context">{p.historicalContext}</CitedText></p></section><section className="static-info-card"><h3><HelpCircle size={14}/> Central problem</h3><p><CitedText record={p} claimKey="central-problem">{p.centralQuestions?.[0]??p.beginnerExplanation}</CitedText></p></section>{p.dateNote&&<section className="static-info-card"><h3><Clock3 size={14}/> Dating note</h3><p><CitedText record={p} claimKey="dating-note">{p.dateNote}</CitedText></p></section>}</div>
        {p.articleSections?<>
          <ArticleToc label={p.name} route={route} href={href}/>
          <ArticleBody sections={p.articleSections} href={href} record={p}/>
