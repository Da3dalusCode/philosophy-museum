...
import {
  ISLAMIC_PALETTE,
  getIslamicSupplementalExhibit,
} from '../../data/museum/islamicSupplementalExhibits';
import {
...
function ExhibitLabel({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  const record = getIslamicSupplementalExhibit(layout.id);
  ...
  return <group position={layout.label.position}>
    <mesh position={[0, 0, -.045]}>
      ...
      <meshStandardMaterial color={ISLAMIC_PALETTE.ink} roughness={.78} metalness={.04}/>
    </mesh>
...
function ExhibitBacking({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group>
    ...
    <mesh position={[0, 2.18, -.445]}>
      ...
      <meshStandardMaterial color={ISLAMIC_PALETTE.ink} roughness={.9} metalness={0}/>
    </mesh>
...
export function IslamicSupplementalExhibits({
  layouts,
  nearbyId,
  onSelect,
}: {
  ...
  return <group userData={{supplementalCollection: 'gallery-10-arabic-islamic-philosophical-worlds-v1'}}>
    {layouts.map((layout) => {
