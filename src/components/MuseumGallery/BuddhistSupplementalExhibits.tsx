...
import {
  BUDDHIST_PALETTE,
  getBuddhistSupplementalExhibit,
} from '../../data/museum/buddhistSupplementalExhibits';
import {
...
function ExhibitLabel({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  const record = getBuddhistSupplementalExhibit(layout.id);
  ...
  return <group position={layout.label.position}>
    <mesh position={[0, 0, -.045]}>
      ...
      <meshStandardMaterial color={BUDDHIST_PALETTE.ink} roughness={.78} metalness={.04}/>
    </mesh>
...
function ExhibitBacking({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group>
    ...
    <mesh position={[0, 2.18, -.445]}>
      ...
      <meshStandardMaterial color={BUDDHIST_PALETTE.ink} roughness={.9} metalness={0}/>
    </mesh>
...
export function BuddhistSupplementalExhibits({
  layouts,
  nearbyId,
  onSelect,
}: {
  ...
  return <group userData={{supplementalCollection: 'gallery-08-buddhist-philosophies-v1'}}>
    {layouts.map((layout) => {
