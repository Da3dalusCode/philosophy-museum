...
import {
  getPhenomenologySupplementalExhibit,
  PHENOMENOLOGY_PALETTE,
} from '../../data/museum/phenomenologySupplementalExhibits';
import {
...
function ExhibitLabel({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  const record = getPhenomenologySupplementalExhibit(layout.id);
  ...
  return <group position={layout.label.position}>
    <mesh position={[0, 0, -.045]}>
      ...
      <meshStandardMaterial color={PHENOMENOLOGY_PALETTE.midnight} roughness={.78} metalness={.04}/>
    </mesh>
...
/**
 * A layered field makes horizon and situated viewpoint legible without adding
 * freestanding decoration or changing the hall architecture.
 */
function HorizonBacking({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group>
    ...
    <mesh position={[0, 2.18, -.445]}>
      ...
      <meshStandardMaterial color={PHENOMENOLOGY_PALETTE.midnight} roughness={.9} metalness={0}/>
    </mesh>
...
export function PhenomenologySupplementalExhibits({
  layouts,
  nearbyId,
  onSelect,
}: {
  ...
  return <group userData={{supplementalCollection: 'gallery-03-work-context-discovery-v1'}}>
    {layouts.map((layout) => {
