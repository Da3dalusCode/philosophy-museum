...
import {
  ...
  getClassicalSouthAsianSupplementalExhibit,
} from '../../data/museum/classicalSouthAsianSupplementalExhibits';
import {
...
function ExhibitLabel({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  const record = getClassicalSouthAsianSupplementalExhibit(layout.id);
  ...
  return <group position={layout.label.position}>
    <mesh position={[0, 0, -.045]}>
      ...
      <meshStandardMaterial color={CLASSICAL_SOUTH_ASIAN_PALETTE.ink} roughness={.78} metalness={.04}/>
    </mesh>
...
function ExhibitBacking({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group>
    ...
    <mesh position={[0, 2.18, -.445]}>
      ...
      <meshStandardMaterial color={CLASSICAL_SOUTH_ASIAN_PALETTE.ink} roughness={.9} metalness={0}/>
    </mesh>
...
export function ClassicalSouthAsianSupplementalExhibits({
  layouts,
  nearbyId,
  onSelect,
}: {
  ...
  return <group userData={{supplementalCollection: 'gallery-07-classical-south-asian-worlds-v1'}}>
    {layouts.map((layout) => {
