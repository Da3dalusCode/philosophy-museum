...
import {
  ANALYTIC_PALETTE,
  getAnalyticSupplementalExhibit,
} from '../../data/museum/analyticSupplementalExhibits';
import {
...
function ExhibitLabel({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group position={layout.label.position}>
    <mesh position={[0, 0, -.045]}>
      ...
      <meshStandardMaterial color={ANALYTIC_PALETTE.ink} roughness={.78} metalness={.04}/>
    </mesh>
...
function AnalyticBacking({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group>
    ...
    <mesh position={[0, 2.18, -.445]}>
      ...
      <meshStandardMaterial color={ANALYTIC_PALETTE.ink} roughness={.9} metalness={0}/>
    </mesh>
...
export function AnalyticSupplementalExhibits({
  layouts,
  nearbyId,
  onSelect,
}: {
  ...
  return <group userData={{supplementalCollection: 'gallery-04-work-context-discovery-v1'}}>
    {layouts.map((layout) => {
