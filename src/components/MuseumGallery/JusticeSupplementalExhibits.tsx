...
import {
  JUSTICE_PALETTE,
  getJusticeSupplementalExhibit,
} from '../../data/museum/justiceSupplementalExhibits';
import {
...
function ExhibitLabel({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  const record = getJusticeSupplementalExhibit(layout.id);
  ...
  return <group position={layout.label.position}>
    <mesh position={[0, 0, -.045]}>
      ...
      <meshStandardMaterial color={JUSTICE_PALETTE.ink} roughness={.78} metalness={.04}/>
    </mesh>
...
function JusticeBacking({layout}: {layout: MuseumSupplementalExhibitLayout}) {
  ...
  return <group>
    ...
    <mesh position={[0, 2.18, -.445]}>
      ...
      <meshStandardMaterial color={JUSTICE_PALETTE.ink} roughness={.9} metalness={0}/>
    </mesh>
...
export function JusticeSupplementalExhibits({
  layouts,
  nearbyId,
  onSelect,
}: {
  ...
  return <group userData={{supplementalCollection: 'gallery-05-justice-democratic-reason-v1'}}>
    {layouts.map((layout) => {
